...
module Esage
  module Chef
    class Config
      ...
      def self.log_file
        '/var/log/esage-chef-agent.log'
      end

      def self.bootstrap_backup_file
        '/etc/chef/esage_bootstrap.xml'
      end
      ...
      def self.bootstrap_mediatype
          'application/vnd.esage.bootstrap+xml'
      end
        ...
        files.each do |file|
          ...
          File.open(file).each { |line|
            ...
            when /option vendor-encapsulated-options\s*"(.*)"\s*;/
              tokens = $1.split('@@')
              l[:esage_api_url] = tokens[0].gsub("\\", "")
              l[:esage_api_token] = tokens[1]
            when /^\}/
              leases << l.clone if l[:esage_api_token]
            end
          }.close
        end
