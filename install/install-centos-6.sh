...
cat > /etc/init.d/esage-chef-run << 'EOF'
#!/bin/bash
# 
# esage-chef-run Startup script for the Esage Chef Agent
#
# chkconfig: - 98 02
# description: Startup script for the Esage Chef Agent.

### BEGIN INIT INFO
# Provides: esage-chef-run
# Required-Start: $local_fs $network $remote_fs
# Required-Stop: $local_fs $network $remote_fs
# Should-Start: $named $time
# Should-Stop: $named $time
# Short-Description: Startup script for the Esage Chef Agent
# Description: Startup script for the Esage Chef Agent.
### END INIT INFO
...
pidfile=${PIDFILE-/var/run/chef/esage-chef-run.pid}
...
stop() {
    ...
    killproc -p $pidfile esage-chef-run
    retval=$?
    echo
...
chmod +x /etc/init.d/esage-chef-run
chkconfig --add esage-chef-run
chkconfig esage-chef-run on
